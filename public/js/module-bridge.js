/**
 * module-bridge.js — CareerConnect iframe SSO (portal token → portal exchange → module session).
 */
(function () {
  "use strict";
  ...
  function establishModuleSession(portalUser) {
    var headers = {
      "Content-Type": "application/json",
      Accept: "application/json",
    ...
    if (window.CAREERCONNECT_SSO_HANDSHAKE_KEY) {
      headers["X-CareerConnect-Sso-Key"] = window.CAREERCONNECT_SSO_HANDSHAKE_KEY;
    }

    return fetch(window.location.origin + "/api/sso/exchange", {
      method: "POST",
      credentials: "include",
      headers: headers,
      body: JSON.stringify({
    ...
    return fetch(PORTAL_ORIGIN + "/api/v1/sso/exchange", {
      ...
      return response.json()
        ...
        .then(function (body) {
          ...
          return establishModuleSession(body.user);
        });
    });
  }
